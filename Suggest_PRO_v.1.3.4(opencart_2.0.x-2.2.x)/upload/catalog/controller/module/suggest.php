    ...
    public function index($setting)
    {
        ...
        if ($data['suggest_status']) {
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/suggest.tpl')) {
                return $this->load->view($this->config->get('config_template') . '/template/module/suggest.tpl', $data);
            } else {
				if (version_compare(VERSION,'2.2.0.0','>=')) {
					return $this->load->view('module/suggest', $data);
				} else {
					return $this->load->view('default/template/module/suggest.tpl', $data);
				}
			}
        }
